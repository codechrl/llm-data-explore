...
yotube_url = st.sidebar.text_input("Input youtube urlc")
submit_button = st.sidebar.button("Submit")
data = fetch_data_from_api(f"{setting.API_ENDPOINT}/api/data/?source=youtube")
...
if submit_button:
    ...
    url = f"{setting.API_ENDPOINT}/api/engine/youtube?background=true&url={yotube_url}"
                        ...
                        try:
                            nodes.append(
                                Node(
                                    ...
                                    size=MIN_SIZE * (1 + graph_elem["size_percentage"]),
                                )
                            )
                        except:
                            pass
